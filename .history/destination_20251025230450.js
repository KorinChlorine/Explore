...
function renderTouristSpots(touristSpots) {
  ...
  const touristCard = document.querySelector(".inner-card");
...
function renderFood(Foods) {
  ...
  const carouselInner = carousel.querySelector(".carousel-inner");
  const indicators = carousel.querySelector(".carousel-indicators");
  const cardContainer = document.querySelector(".food-card"); // the card with background
  ...
  // --- Set initial background via CSS variable ---
  if (Foods.length > 0) {
    cardContainer.style.setProperty("--bg-url", `url(${Foods[0].Image})`);
  }

  // --- Update background on slide change ---
  carousel.addEventListener("slid.bs.carousel", (event) => {
    const activeItem = event.relatedTarget;
    const bgUrl = activeItem.getAttribute("data-bg");

    // Update CSS variable for ::before pseudo-element
