...
function showCountries(continent, page) {
  ...
  pageCountries.forEach(country => {
    ...
    card.querySelector(".see-more-btn").addEventListener("click", () => {
      displayCities(continent, country);
    });
...
// ====== Cities ======
function displayCities(continent, country) {
  currentCountry = country;
  aboutSection.style.display = "none";
  countrySection.style.display = "none";
  citySection.style.display = "block";

  setupCityPagination(continent, country);
  showCities(continent, country, 1);
}

function showCities(continent, country, page) {
  ...
  pageCities.forEach(city => {
    ...
    // ✅ Fixed: Pass the correct continent name
    card.querySelector(".see-more-btn").addEventListener("click", (e) => {
      e.stopPropagation();
      window.location.href = `destination.html?continent=${continent.continent}&place=${city.id}`;
    });
