...
function createReviews(certainPlace) {
  ...
  function updateCard() {
    ...
    setTimeout(() => {
      ...
      reviewerContainer.classList.add(
        "reviewer-info",
        "mt-4",
        "d-flex",
        "align-items-center",
      ...
      profile.classList.add("review-profile");
      ...
      info.innerHTML = `
        ...
        <small class="text-secondary">Verified Guest</small>
      `;

      const oldReviewer = document.querySelector(".reviewer-info");
      if (oldReviewer) oldReviewer.remove();

      reviewerContainer.appendChild(profile);
      reviewerContainer.appendChild(info);
      text.insertAdjacentElement("afterend", reviewerContainer);

      // === Update dots ===
      dots.forEach((dot, i) => {
        dot.style.background = i === currentIndex ? "#f39c12" : "#ccc";
      });
  ...
  prevBtn.addEventListener("click", () => {
    currentIndex =
      (currentIndex - 1 + certainPlace.reviews.length) %
      certainPlace.reviews.length;
    updateCard();
  });

  // === Style Dots & Controls Section (below card) ===
  dotsContainer.classList.add("review-dots");
  ...
  [prevBtn, nextBtn].forEach(btn => {
    btn.addEventListener("mouseenter", () => (btn.style.color = "#f39c12"));
