...
function renderFood(Foods) {
  ...
  Foods.forEach((food, index) => {
    ...
    item.innerHTML = `
      ...
      <div class="card-navigation-button position-relative">
       <button class="History active">About</button>
       <button class="History active">Where to find it</button>
       </div>
      <p class="info food-info position-relative w-100">${food.Description}</p>
    `;
...
function renderActivities(activities) {
  const carouselInner = document.querySelector("#ActivityCarousel .carousel-inner");
  const indicators = document.querySelector("#ActivityCarousel .carousel-indicators");
  const imageScroll = document.querySelector(".image-scroll-activity");

  activities.forEach((activity, index) => {
    // Indicator
    const indicator = document.createElement("button");
    indicator.type = "button";
    indicator.dataset.bsTarget = "#ActivityCarousel";
    indicator.dataset.bsSlideTo = index;
    if (index === 0) indicator.classList.add("active");
    indicators.appendChild(indicator);
    ...
    item.innerHTML = `
      <h3 class="fw-bold activity-title">${activity.name}</h3>
      <p><i class="bi bi-geo-alt-fill"></i> ${activity.location}</p>
      <p><i class="bi bi-currency-dollar"></i> ${activity.price}</p>
      <p><i class="bi bi-person-fill"></i> ${activity.age}</p>
  ...
  // Update image on carousel slide
  const activityCarousel = document.querySelector("#ActivityCarousel");
  activityCarousel.addEventListener("slid.bs.carousel", (event) => {
    const activeIndex = event.to;
...
const activities = [
  {
    ...
    age: "12+ years old",
    description: "Explore the breathtaking underwater world of the Great Barrier Reef with our guided scuba diving tours...",
    image: "https://plus.unsplash.com/premium_photo-1661964149725-fbf14eabd38c?ixlib=rb-4.1.0&auto=format&fit=crop&q=80&w=1470"
  }, 
  {
    name: "Scuba did Adventure",
    location: "Great Barrier Reef, Australia",
    price: "$120 per person",
    age: "1+ years old",
    description: "Explore the breathtaking underwater world of the Great Barrier Reef with our guided scuba diving tours...",
